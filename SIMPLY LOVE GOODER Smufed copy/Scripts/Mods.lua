...
-- Judgment Font List
	judgmentFontList = { 'Love' , 'Tactics' , 'Chromatic' , 'Deco' , 'GrooveNights' , 'Wellbut' , 'ITG2' , 'Konigsberg' , 'Neon' , 'Gameplay' , 'Voltex' , 'Quantify' , 'VCR', 'Nulshock', 'Outer Limits', 'Technique', 'Groovy', 'sm-ssc' , 'Bebas Vertical' , 'Bold' , 'Classic' , 'Classic Beginner' , 'die' , 'DJMax' , 'Droidiga' , 'japanese' , 'Mariokart' , 'Metallikkii' , 'Misfits' , 'NJSRT' , 'NJSRT2' , 'NJSRT2 Chroma' , 'RAKKI RAVE' , 'Speed Trakkii' , 'Wellbut' , 'Wonder Parade' , 'Wonder Parade Chroma' , 'Yaz Chroma' , 'Yaz Chroma Verbose' , 'Yaz Gradient' , 'Yaz Gradient Verbose' , 'Yaz Solid' , 'Yaz Solid Verbose' }
...
-- These will be the option rows available on the [nth] option screen. The 'NextScreen' row will be automatically added as long as there is more than 1 option screen.
	playerOptions = {}
	playerOptions[1] = { 'SpeedType','SpeedNumber','Mini','Perspective','NoteSkin','Turn','Voice','JudgmentFont','Rate' }
	playerOptions[2] = { 'Turn','Accel','Scroll','Effect','Appearance','Handicap','InsertTaps','InsertOther','Hide','Ghost','LifeBar','Compare' }
	if FUCK_EXE then
	playerOptions[3] = { 'Invisible','GlobalModTimer','GlobalModTimerMult','GlobalModTimeOffset','Grain','Beat','BeatY','BeatZ','DrunkZ','TanTipsy','TanDrunk','LongBoy','Square','SquareZ','Digital','Drunk','DrunkSize','DrunkSpeed','DrunkOffset','ZigZag','ZigZagZ','Sawtooth','SawtoothZ','DigitalZ','TanDigital','TanDigitalZ','ParabolaX','ParabolaY','ParabolaZ','TanExpand','TanBumpy','AttenuateX','AttenuateY','AttenuateZ','SpiralX','SpiralY','SpiralZ' }
	playerOptions[4] = { 'StealthNotITG','HiddenNotITG','SuddenNotITG','ConfusionNotITG','HoldNotITG' }
	end
	playerOptions.Edit = { 'SpeedType','SpeedNumber','Mini','Perspective','NoteSkin','Turn','Invisible','Grain','Beat','BeatY','BeatZ','DrunkZ','TanTipsy','TanDrunk','LongBoy','Square','SquareZ','Digital','StealthNotITG','HiddenNotITG','SuddenNotITG','ConfusionNotITG','HoldNotITG','Drunk','DrunkSize','DrunkSpeed','DrunkOffset','ZigZag','ZigZagZ','Sawtooth','SawtoothZ','DigitalZ','TanDigital','TanDigitalZ','ParabolaX','ParabolaY','ParabolaZ','TanExpand','TanBumpy','AttenuateX','AttenuateY','AttenuateZ','SpiralX','SpiralY','SpiralZ' }
	ShowAllInRow = false
...
ModsMaster.StealthNotITG =	{ name = 'Stealth Mods', modlist = {'StealthRed','StealthGreen','StealthBlue'}, default = 'no StealthGlowRed, no StealthGlowGreen, no StealthGlowBlue', mods = {'50% stealth, StealthGlowRed','50% stealth, StealthGlowGreen','50% stealth, StealthglowBlue'}, }
ModsMaster.HiddenNotITG = 	{ name = 'Hidden Mods', modlist = {'HiddenRed','HiddenGreen','HiddenBlue'}, default = 'no hiddenred, no hiddengreen, no hiddenblue', mods = {'HiddenRed','HiddenGreen','HiddenBlue'} }
ModsMaster.SuddenNotITG = 	{ name = 'Sudden Mods', modlist = {'SuddenRed','SuddenGreen','SuddenBlue'}, default = 'no Suddenred, no Suddengreen, no Suddenblue', mods = {'SuddenRed','SuddenGreen','SuddenBlue'} }
ModsMaster.BlinkNotITG = 	{ name = 'Blink Mods', modlist = {'BlinkRed','BlinkGreen','BlinkBlue'}, default = 'no Blinkred, no Blinkgreen, no Blinkblue', mods = {'BlinkRed','BlinkGreen','BlinkBlue'} }
ModsMaster.ConfusionNotITG = 	{ name = 'Confusion', modlist = {'ConfusionX','ConfusionY','ConfusionZ'}, default = 'no ConfusionX, no ConfusionY, no confusion', mods = {'ConfusionX','ConfusionY','Confusion'} }
ModsMaster.HoldNotITG = 	{ name = 'Hold Manipulation', modlist = {'Spiral Holds','Dizzy Holds','Hold Type'}, default = 'no spiralholds, no dizzyholds, no holdtype', mods = {'100% spiralholds','100% dizzyholds','100% holdtype'} }
...
-- NotITG Mods
ModsMaster.BeatY =			{ float = true }
ModsMaster.BeatZ =			{ float = true }
ModsMaster.TanTipsy =		{ float = true }
ModsMaster.TanDrunk =		{ float = true }
ModsMaster.Mini =			{ float = true }
ModsMaster.Grain =			{ float = true }
ModsMaster.Square =			{ float = true }
ModsMaster.SquareZ =		{ float = true }
ModsMaster.Bounce = 		{ float = true }
ModsMaster.BeatZOffset =	{ float = true }
ModsMaster.BeatZPeriod =	{ float = true }
ModsMaster.BeatZMult =		{ float = true }
ModsMaster.DrunkZ =			{ float = true }
ModsMaster.LongBoy =		{ float = true }
ModsMaster.Digital = 		{ float = true }
ModsMaster.DrunkSize =		{ float = true }
ModsMaster.DrunkSpeed =		{ float = true }
ModsMaster.DrunkOffset =	{ float = true }
ModsMaster.ZigZag =			{ float = true }
ModsMaster.ZigZagZ =		{ float = true }
ModsMaster.Sawtooth =		{ float = true }
ModsMaster.SawtoothZ =		{ float = true }
ModsMaster.DigitalZ =		{ float = true }
ModsMaster.TanDigital = 	{ float = true }
ModsMaster.TanDigitalZ =	{ float = true }
ModsMaster.ParabolaX =		{ float = true }
ModsMaster.ParabolaY =		{ float = true }
ModsMaster.ParabolaZ =		{ float = true }
ModsMaster.TanExpand =		{ float = true }
ModsMaster.TanBumpy =		{ float = true }
ModsMaster.AttenuateX =		{ float = true }
ModsMaster.AttenuateY =		{ float = true }
ModsMaster.AttenuateZ =		{ float = true }
ModsMaster.SpiralX =		{ float = true }
ModsMaster.SpiralY =		{ float = true }
ModsMaster.SpiralZ =		{ float = true }
ModsMaster.GlobalModTimer =	{ float = true }
ModsMaster.GlobalModTimerMult = { float = true }
ModsMaster.GlobalModTimeOffset = { float = true }
...
function NextScreenOption()
	...
	t.SaveSelections = function(self, list, pn)
			if list[1] then
                if SCREENMAN:GetTopScreen():GetName() == 'ScreenPlayerDetailedOptions' then
                    nextScreen = 'ScreenDetailedStage'
                else
